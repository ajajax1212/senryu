...
import type { Action, GameSettings, GameState, Mode, Phase } from '../engine/types';
...
export function Local({ mode, onBack }: { mode: Mode; onBack: () => void }) {
  const [game, dispatch] = useReducer(localReducer, null);
  const [draft, setDraft] = useState<Draft & { key: string }>({ key: '' });

  function start(names: string[], settings: GameSettings) {
    dispatch({ type: 'START_GAME', mode, settings: { ...settings, passAndPlay: true }, names });
  }

  if (!game) return <Setup mode={mode} online={false} onStart={start} onBack={onBack} />;
